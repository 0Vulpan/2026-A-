...
void PID_Control_Init(void)
{
    ...
    Ramp_Init(&speed_ramp, 280.0f, 1.0f, 0.05f);                  // 目标280RPM，3秒爬坡
...
{
    EncoderSpeed_TimerCallback();  // 更新编码器

    motor_pid.setpoint = Ramp_Update(&speed_ramp);  // 平滑目标

    float raw_rpm = EncoderSpeed_GetRPM();
    float filtered_rpm = LPF_Update(&speed_filter, raw_rpm);

    float output = PID_Compute(&motor_pid, filtered_rpm, 0.05f);  // 50ms周期

    Set_PWM_Duty((int16_t)output);  // 输出PWM
}
/*
